...
- [怎么开机/亮屏](#怎么开机亮屏)
- [为什么插线不亮屏](#为什么插线不亮屏)
- [为什么按闪电插线不亮屏](#为什么按闪电插线不亮屏)
- [为什么拔掉手机就黑屏](#为什么拔掉手机就黑屏)
- [为什么显示“K2 DFU”](#为什么显示k2-dfu)；

### 怎么开机/亮屏

K2左右侧的Type-C公头和母座皆可上电，可以将Type-C数据线插入母座开机/亮屏，也可以将K2的公头插入插头开机/亮屏。

<font color="blue">**注意**</font>：无论您使用的是哪一种类型的线材（AC线或是CC线），或者是插入插头，推荐您始终使用长按 **`+`** 上电。

<font color="orange">**备注**</font>：

> K2官方提供另一种开机方式：按 **`OK`** 开机。但是此开机方式存在许多限制，通常不推荐您这么做。
>
> 如果您只考虑将K2作为一个充电监视器，那么不需要做任何开机操作，将其连入原先充电电路即可；如果执行开机操作可能会导致充电功率极低、协议错误，详见[为什么接K2后充电功率很低](#为什么接K2后充电功率很低)。
...
### 目录
- [为什么不插设备也有功率](#为什么不插设备也有功率)
- [为什么显示功率小于充电器标注功率](#为什么显示功率小于充电器标注功率)
- [为什么第一页右下角显示的功率和实际功率不一样](#为什么第一页右下角显示的功率和实际功率不一样)
- [为什么第一页右下角绿/灰框里的功率和我手机支持的功率不一样](#为什么第一页右下角绿灰框里的功率和我手机支持的功率不一样)
- [为什么接K2后充电功率很低](#为什么接K2后充电功率很低)
...
如果设备/手机正常激活快充且设备/手机接近满电，则为正常现象，参考[为什么显示功率小于充电器标注功率](#为什么显示功率小于充电器标注功率)。
...
- [怎么测快充协议](#怎么测快充协议)
- [怎么诱骗快充协议](#怎么诱骗快充协议)
- [为什么一测快充协议就黑屏](#为什么一测快充协议就黑屏)
- [为什么协议检测/诱骗不能接手机/设备](#为什么协议检测诱骗不能接手机设备)
- [能不能通过协议诱骗强行高功率快充](#能不能通过协议诱骗强行高功率快充)
- [能不能通过协议诱骗把充电头的A协议变成手机支持的B协议](#能不能通过协议诱骗把充电头的A协议变成手机支持的B协议)
- [为什么检测出来的协议与充电头标注的不一样](#为什么检测出来的协议与充电头标注的不一样)
- [为什么第一页右下角显示的协议和手机实际协议不一样](#为什么第一页右下角显示的协议和手机实际协议不一样)
- [什么是“PD加速”](#什么是PD加速)
...
各版本下界面有所不同，V2.9及以下版本如需更改虚拟E-Marker[^7]，需要单独去**设置->虚拟EMARK**更改；V3.1及以上版本可以快速选择虚拟E-Marker；V3.4版本增加PD加速[^8]开关。
...
<font color="blue">**注意**</font>：虽然K2支持虚拟E-Marker，但是最优的检测方法还是通过充电头官方原装数据线连接K2母座，或者K2公头连接充电头时母座插入充电头官方原装数据线**并悬空**（用于充电头检测数据线的E-Marker[^7]），此时检测到的协议最为准确。自动检测协议不可以使用HID供电，当遇到特殊情况： 黑屏，无限重启的充电头或充电宝，可接HID供电测完。检测过程中，K2会**重启**，这是正常现象。
...
- [怎么自定义表盘和开机图](#怎么自定义表盘和开机图)
- [为什么上传了表盘却没有显示](#为什么上传了表盘却没有显示)
- [为什么表盘页显示“无主题参数”](#为什么表盘页显示无主题参数)
- [为什么表盘页显示“无主题图”](#为什么表盘页显示无主题图)
...
<font color="blue">**注意**</font>：每次准备更新时都需要将K2进入DFU模式[^6]，每次更新结束后都会退出DFU模式，关于DFU模式详见[怎么进入DFU模式](#怎么进入dfu模式5)。
...
- [E-Marker是什么/干什么](#E-Marker是什么干什么)
- [“inbuilt emark”是什么](#inbuilt-emark是什么)
- [怎么读取E-Marker](#怎么读取E-Marker)
- [怎么导出/导入E-Marker](#怎么导出导入E-Marker)
...
[^1]: 例如这样
[^2]: 指可以将K2的实时数据传输到电脑的方法，详见[如何连接上位机](#如何连接上位机)
